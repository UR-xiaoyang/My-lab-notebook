...
try:
    ...
    if PLOT_AVAILABLE and not df.empty:
        ...
        plt.title('目标变量 Survived 分布 (0=未生存, 1=生存) - train.csv')
        plt.xlabel('是否生存')
        plt.ylabel('数量')
        plt.show()
        ...
        for col in vis_numerical_features:
            if col in df.columns:
                ...
                # histplot 会自动忽略NaN值
                sns.histplot(df[col].dropna(), kde=True) # 使用 dropna() 明确告知处理方式
                plt.title(f'{col} 分布 (原始数据)')
                plt.xlabel(col)
                plt.ylabel('频率')
                plt.show()
            else:
        ...
        for col in vis_categorical_features:
            if col in df.columns:
                plt.figure(figsize=(8, 5))
                # countplot 默认会为 NaN 创建一个单独的条目 (如果存在且pandas未处理)
                # 或者我们可以先填充一个特定的占位符如果想明确显示
                sns.countplot(x=df[col].fillna('Missing'), palette='viridis')
                plt.title(f'{col} 分布 (原始数据) - Missing表示缺失值')
                plt.xlabel(col)
                plt.ylabel('数量')
                plt.show()
            else:
        ...
        if actual_numerical_features_for_corr:
            ...
            # .corr() 默认会成对排除NaN (pairwise deletion)
            correlation_matrix = df[actual_numerical_features_for_corr].corr()
            sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', fmt=".2f")
            plt.title('数值特征相关性热图 (原始数据)')
            plt.show()
        else:
            print("警告: 没有可用于绘制相关性热图的数值特征。")

        # 5. 特征与 Survived 的关系
        print("\n特征与 Survived 的关系 (原始数据):")
        # Sex vs Survived
        if 'Sex' in df.columns and 'Survived' in df.columns:
            plt.figure(figsize=(6, 4))
            sns.countplot(x=df['Sex'].fillna('Missing'), hue='Survived', data=df, palette='muted')
            plt.title('性别 vs 生存状态 (原始数据)')
            plt.xlabel('性别')
            plt.ylabel('数量')
            plt.show()
        ...
        if 'Pclass' in df.columns and 'Survived' in df.columns:
            plt.figure(figsize=(6, 4))
            sns.countplot(x=df['Pclass'].fillna('Missing'), hue='Survived', data=df, palette='pastel')
            plt.title('船舱等级 vs 生存状态 (原始数据)')
            plt.xlabel('船舱等级')
            plt.ylabel('数量')
            plt.show()
        ...
        if 'Embarked' in df.columns and 'Survived' in df.columns:
            plt.figure(figsize=(6, 4))
            sns.countplot(x=df['Embarked'].fillna('Missing'), hue='Survived', data=df, palette='dark')
            plt.title('登船港口 vs 生存状态 (原始数据)')
            plt.xlabel('登船港口')
            plt.ylabel('数量')
            plt.show()
        ...
        if 'Fare' in df.columns and 'Survived' in df.columns:
            ...
            plt.title('票价 (对数刻度) 分布 vs 生存状态 (原始数据)')
            plt.xlabel('票价 (对数刻度)')
            plt.ylabel('密度')
            plt.show()
